...
export default function NosotrosPageClient({ lang }: Props) {
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'instant' })
  }, [])

  const t        = (translations[lang] as any).nosotros
  const tAbout   = (translations[lang] as any).about
    ...
    <div style={{ background: 'oklch(0.12 0.025 255)', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="relative pt-16 h-[60vh] min-h-[400px] overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 lg:px-8 h-full flex flex-col justify-end pb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs tracking-[0.3em] uppercase text-[oklch(0.78_0.12_75)]">
              ATREVIA Consultores
            </span>
          </div>
          <h1
          ...
          <p className="text-base lg:text-lg text-[oklch(0.65_0.015_255)] max-w-2xl">{t.heroSub}</p>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start mb-24">
          <div>
            <div className="flex items-center gap-4 mb-6">
              <div className="w-8 h-px bg-[oklch(0.78_0.12_75)]" />
              <h2 className="text-2xl lg:text-3xl font-light text-[oklch(0.95_0.01_80)]" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
                {t.introTitle}
              </h2>
            </div>
            <p className="text-base text-[oklch(0.65_0.015_255)] leading-relaxed">{t.introText}</p>
          </div>
          <div className="rounded p-8" style={{ background: 'oklch(0.16 0.025 255)', border: '1px solid oklch(0.22 0.03 255)' }}>
            <div className="flex flex-col gap-6">
              {[
                { flag: '🇮🇹', label: tAbout.italyLabel, desc: tAbout.italyDesc },
                { flag: '🇵🇪', label: tAbout.peruLabel,  desc: tAbout.peruDesc  },
              ].map(({ flag, label, desc }) => (
                <div key={flag}>
                  <div className="flex items-center gap-4 mb-4">
                    <span className="text-4xl">{flag}</span>
                    <div>
                      <p className="text-xs font-semibold text-[oklch(0.95_0.01_80)] uppercase tracking-wider mb-1">{label}</p>
                      <p className="text-sm text-[oklch(0.55_0.015_255)]">{desc}</p>
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm text-[oklch(0.65_0.015_255)]">{tAbout.text2}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-24">
          ...
          <p className="text-base text-[oklch(0.65_0.015_255)] leading-relaxed mb-10 max-w-3xl">{t.commitmentText}</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {t.pillars.map((p: { title: string; desc: string }, i: number) => (
              <div key={i} className="p-6 rounded" style={{ background: 'oklch(0.16 0.025 255)', border: '1px solid oklch(0.22 0.03 255)' }}>
                <div className="w-8 h-8 rounded flex items-center justify-center mb-4"
                  style={{ background: 'oklch(0.78 0.12 75 / 0.12)', border: '1px solid oklch(0.78 0.12 75 / 0.3)' }}>
                  <CheckCircle className="w-4 h-4 text-[oklch(0.78_0.12_75)]" />
                </div>
                <h3 className="text-base font-semibold text-[oklch(0.90_0.01_80)] mb-2" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>{p.title}</h3>
                <p className="text-sm text-[oklch(0.60_0.015_255)] leading-relaxed">{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Global approach */}
        <div className="relative rounded overflow-hidden mb-24 p-10 lg:p-14"
          ...
          <p className="text-base text-[oklch(0.65_0.015_255)] leading-relaxed max-w-3xl">{t.globalText}</p>
        </div>
        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {t.whyItems.map((item: { label: string; desc: string }, i: number) => (
              <div key={i} className="flex items-start gap-4 p-5 rounded transition-all duration-300 hover:-translate-y-1"
                ...
                <div>
                  <p className="text-sm font-semibold text-[oklch(0.88_0.01_80)] mb-1">{item.label}</p>
                  <p className="text-sm text-[oklch(0.60_0.015_255)] leading-relaxed">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* CTA */}
        <div className="relative rounded overflow-hidden p-10 lg:p-16 text-center"
          style={{ background: 'oklch(0.16 0.025 255)', border: '1px solid oklch(0.25 0.03 255)' }}>
          <div className="absolute inset-0 pointer-events-none"
          ...
          <h3 className="text-2xl lg:text-3xl font-light text-[oklch(0.95_0.01_80)] mb-4 text-balance"
            style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>{t.ctaTitle}</h3>
          <p className="text-sm text-[oklch(0.65_0.015_255)] mb-8 max-w-xl mx-auto leading-relaxed">{t.ctaText}</p>
          <a
            href="#contacto"
      ...
      <footer className="mt-8 py-8 text-center" style={{ borderTop: '1px solid oklch(0.18 0.025 255)' }}>
        <p className="text-xs text-[oklch(0.40_0.015_255)]">
